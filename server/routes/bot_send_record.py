...
## Env: BOT_TOKEN, BOT_SEND_MODE, APP_BASE_URL (for absolute URLs)
...
async def send_record(payload: dict = Body(...)):
    # Debug входящего payload
    try:
      print("[BOT] incoming payload:", payload)
    except Exception:
      pass

    chat_id  = str(payload.get("chat_id") or "").strip()
    owner_id = str(payload.get("owner_uid") or "").strip()
    file_url = (payload.get("file_url") or "").strip()
    room_id  = (payload.get("room_id") or "").strip()

    if not chat_id and owner_id:
        chat_id = owner_id

    if not file_url:
        print(f"[BOT] missing file_url in payload: {payload}")
    ...
    if BOT_SEND_MODE == "video":
        ...
        return {"ok": bool(res), "mode": "video", "url": file_url_abs}## Back-compat endpoint /bot/send_record with send mode switch 'link' or 'video'
## Env: BOT_TOKEN, BOT_SEND_MODE, APP_BASE_URL (for absolute URLs)
